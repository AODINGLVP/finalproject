...
func _run() -> void:
	...
	print("ARENA_SMOKE active_runners=%d player_hud_chars=%d complex_alive=%s" % [active_runners, hud_text.length(), str(complex_alive)])
	game.free()
	await process_frame
	await process_frame
	_quit_with_result.call_deferred(0 if active_runners == 3 and hud_text.length() > 20 and complex_alive else 1)
