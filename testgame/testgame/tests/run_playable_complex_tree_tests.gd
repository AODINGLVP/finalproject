...
const TREE_PATH := "res://behavior_trees/playable_complex_enemy_213.tres"
...
func _initialize() -> void:
	...
	if tree != null:
		_expect(tree.nodes.size() == 213, "playable complex tree has exactly 213 nodes")
		_expect(tree.validate_tree().is_empty(), "playable complex tree validates")
		_expect(tree.blackboard_schema != null and tree.blackboard_schema.entries.size() == 17, "typed schema describes all tactical inputs")
		...
		_expect(priority != null and tree.get_children_of(priority.id).size() == 9, "reactive selector has nine ordered tactical layers")
