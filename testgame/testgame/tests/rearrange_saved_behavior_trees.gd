...
func _rearrange_and_verify(path: String, tree: BTTreeResource) -> void:
	...
	_expect(_parent_child_vertical_gap(tree) >= BTTreeLayout.VERTICAL_STEP - BTTreeLayout.CARD_SIZE.y - 0.1, "%s leaves a clear vertical gap between every parent and child" % path)
