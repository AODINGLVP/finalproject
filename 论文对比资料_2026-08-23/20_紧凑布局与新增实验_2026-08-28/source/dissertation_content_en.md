...
This project does not implement EdgeLens or edge bundling and does not change connection routes. Readable Edge Overlay only allows a line to appear through a semi-transparent card background and uses text outlines to protect glyphs. Its method is simpler and its scope is more limited. Taken together, previous work has considered layout, zoom, focus, highlighting, connections and recent visual workflows separately, but it has not compared this combination in a genuine Godot behaviour-tree editor. The research gap in this project is not the proposal of a completely new graph algorithm. Instead, it turns five approaches into practical features that can be enabled, disabled and reset, and then uses real game trees, different scales and different screen sizes to examine the change and cost produced by each feature.
...
Five groups of automated tests were rerun on the current version. They cover the execution system, editor operations, the basic game scene, complex enemy behaviour and the complete five-enemy flow. Every test completed normally, and there were no program errors or crashes during the run. The exact numbers for each group are shown in Table 5.1.

| Validation suite | Passed/total | Principal scope |
| --- | ---: | --- |
| Execution-system tests | 154/154 | Node execution, Decorators, blackboards, save and reload |
| Editor-function tests | 337/337 | Creating and editing nodes, selection, dragging, display optimisation and reset |
| Basic game tests | 40/40 | Game actors can receive behaviour-tree actions and the basic scene works |
| Complex game-behaviour tests | 34/34 | Enemy combat, movement, recovery, obstacles and ladders |
| Complete game-flow tests | 215/215 | Five tree scales load, enemies can be defeated and the game can finish |
| Total | 780/780 | All automated checks in this run |

Table 5.1 Plugin-function and game-validation results for the current version

Each of the five behaviour trees controls its corresponding enemy, and the game can be completed. The following display results are therefore based on real resources that load and execute correctly.

## 5.2 Formal display data and safety checks

The formal experiment produced 225 before-and-after comparisons. In other words, the same display optimisation was tested repeatedly under different node counts, screen sizes and target nodes. Each pair changed only one display optimisation switch, while the other conditions stayed the same. This makes it possible to compare the change caused by that one function in that situation.

Every pair passed the feature and structure checks. The behaviour-tree connections, execution order and saved positions did not change. The Smart test restores the dragged node after recording, so coordinate equality here means that automatic avoidance did not write to the resource. In normal editing, a position deliberately created by the user is still saved.

| Display optimisation | Before activation | After activation | Main change | Point to watch |
| --- | --- | --- | --- | --- |
| Smart Drag Reflow | Dragging creates overlap, with a mean overlap area of 93,834.90 px² | Overlap area is reduced to 0 in 45/45 cases | Removes the deliberately created node occlusion | Moves a mean of 2.40 other cards; median total movement is 208.54 px |
...
| Readable Edge Overlay | Card background is fully opaque, so a line through a card can be hidden | Background opacity is reduced to 0.72 while text remains opaque | Median controlled occluded segment is 199.50 px and the route is unchanged | Helps local line occlusion, but does not reduce node count |
| Related Node Focus | Unrelated nodes keep normal brightness | All unrelated nodes are dimmed | Mean reduction of 58.51% in fully bright viewport candidates | Not suitable for comparing unrelated branches at the same time |
| Fisheye Focus | Mean target width is 119.43 px in overview | Mean target width is 224.55 px | Local target becomes much larger and restores more fields | New local overlap in 25/45 cases and new visual hierarchy occlusion in 28/45 |

Table 5.2 Principal result of each feature relative to its off state

Each row in Table 5.2 measures a different task, so the values should not be added into a single total score. The following sections explain each display optimisation separately. All before-and-after figures use the 241-node behaviour tree, the approximately 27-inch screen profile and the same test target. The screenshots include only the behaviour-tree editing canvas, not the Inspector panel.

## 5.3 Before-and-after comparison of the five features

### 5.3.1 Smart Drag Reflow

Smart Drag Reflow handles new occlusion caused when the user drags a node. In the experiment, the script drags the same leaf node close to another node and deliberately creates overlap. With the function disabled, the overlap remains. With the function enabled, nearby cards move away without changing the saved behaviour-tree resource.
...
Figure 5.1 shows the difference at the same dragged position. After activation, the overlap area is reduced to zero in all 45 cases. The function does not force the whole tree into a fixed layout; it moves a small part of the structure related to the current drag. On average, 2.40 other cards move; the median is one card and the maximum is seven cards. The median total movement of all other cards is 208.54 px, and the maximum is 906.37 px. This shows that the function is effective for drag occlusion, while the user still needs to accept that a small number of surrounding cards can move.

### 5.3.2 Adaptive Zoom Detail

Adaptive Zoom Detail addresses the overview problem. When a behaviour tree is zoomed out, full cards occupy a large amount of space. If everything is simply scaled down, text becomes difficult to read. This function hides secondary fields and uses more compact cards at low zoom.
...
In Figure 5.2, the on state keeps node identity, type and main structural information, but it no longer shows every parameter. Across the experiment, total card area falls by a mean of 45.14%, and the number of fields falls by a mean of 43.61%. No new case appears where a parent is visually below its child. This makes it suitable as the basic overview feature for large trees: the user first sees the structure and branch range, then zooms in to read exact parameters.

### 5.3.3 Readable Edge Overlay

Readable Edge Overlay addresses the local problem where a connection is hidden by a card. The experiment has two parts. First, it scans the real viewport and records how many visible connections pass through non-endpoint cards. Second, it uses the same real parent-child connection and a non-endpoint card to create a controlled occlusion case, and then compares the off and on states.
...
The natural viewport scan finds a mean of 3.62 visible connections occluded by non-endpoint cards in each case. In the controlled case, the median hidden segment length is 199.50 px. After activation, the card background becomes translucent, but text and icons remain opaque, and the route of the connection does not change. This means the feature can reveal part of a segment that was covered by a card background. Its effect is local: it does not reduce the number of nodes, and it does not make the whole tree less dense.

### 5.3.4 Related Node Focus

Related Node Focus is used to inspect the structural range of the selected node. After activation, selected nodes use a white border, ancestors and descendants use a yellow border, siblings use a green border, and unrelated nodes and edges are dimmed. Because the current editor supports box selection, the experiment also includes one case where two nodes are selected together.
...
The results show that all unrelated nodes are dimmed correctly, and the number of fully bright candidates in the viewport falls by a mean of 58.51%. Node positions, card area and edge routes remain unchanged. The user can still see where the complete tree is, but attention is more easily directed toward the parents, children and siblings related to the current selection.

### 5.3.5 Fisheye Focus

Fisheye Focus is mainly used in a zoomed-out overview. The off state keeps Adaptive Zoom Detail enabled. The on state adds fisheye magnification around the pointer and makes far nodes smaller and dimmer. This tests whether the user can temporarily read one local area without leaving the overview.
...
After activation, mean target width rises from 119.43 px to 224.55 px, and more fields are restored. This change is visually clear, especially when a large tree has been strongly reduced. However, fisheye also has the clearest cost: 25/45 cases introduce new local overlap, and 28/45 introduce new visual hierarchy occlusion. Tree structure and execution order remain unchanged, but the view no longer guarantees global tidiness. Fisheye is therefore more useful for brief local inspection than as a continuously enabled main layout.

## 5.4 Results across node scales

Table 5.3 summarises the results across different node scales. To keep the table readable, each display optimisation is represented by the metric that best matches its purpose. Adaptive uses card-area reduction, Related uses the reduction in fully bright viewport candidates, Fisheye uses target-width increase, Smart uses the number of other cards moved and their total movement, and Overlay uses the ratio of visible edges naturally occluded by cards.

| Resource nodes | Adaptive area reduction | Related fully bright candidate reduction | Fisheye width increase | Smart mean moved cards | Smart mean total movement | Overlay natural occluded-edge ratio |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
Table 5.3 shows that Adaptive becomes more apparent from 121 nodes upward, because large trees need more area reduction in overview. Related becomes stronger from 61 to 241 nodes, which suggests that unrelated branches become more distracting as the tree becomes more complex. The 364-node tree has a lower Related value than the 241-node tree mainly because the fixed target branch is different and the visible ratio of related to unrelated nodes changes.

Fisheye has the clearest scale trend. This does not mean the on-state card grows without limit. Instead, the off-state target becomes smaller as larger trees are zoomed out, while the on-state target returns to roughly 220 px. Smart removes drag overlap at every scale, but the number of moved cards is not monotonic because it depends on the local branch around the drag point. Overlay is highest on the 364-node tree and lowest on the 241-node tree, showing that it mainly depends on the current viewport and connection path rather than on node count alone.

## 5.5 Results across screen sizes

Table 5.4 summarises the results across three screen sizes. The same behaviour trees and target nodes are used; only the behaviour-tree editor viewport size changes.

| Screen profile | Adaptive area reduction | Related fully bright candidate reduction | Fisheye width increase | Smart mean moved cards | Overlay natural occluded-edge ratio |
| --- | ---: | ---: | ---: | ---: | ---: |
| 15.94 in | 57.14% | 54.01% | 258.98% | 2.40 | 0.1682 |
| 26.96 in | 42.61% | 59.61% | 104.90% | 2.40 | 0.1735 |
| 31.55 in | 35.67% | 61.90% | 75.90% | 2.40 | 0.1825 |
...
Table 5.4 shows that a small screen does not make every function more effective. Adaptive and Fisheye have the largest relative effect on the small screen because the overview must shrink the behaviour tree more strongly. Adaptive first reduces global area, and Fisheye can then temporarily enlarge the current target. Their role is therefore to compensate for limited space on a small screen.

Related has a higher reduction on the medium and large screens. A larger screen can show more branches at the same time, and many of those branches are unrelated to the selected node. Related does not move these nodes; it only dims them. This makes it better at using the global space of a large screen. Smart moves the same mean number of cards on all three screens, so this test shows that avoidance is mainly controlled by the drag position. Overlay's natural occlusion ratio rises slightly with screen size because a larger view contains more visible connections and can therefore include more lines passing through cards.

## 5.6 Structured developer evaluation

| Recommended order | Display optimisation | Most suitable task and environment | Advantage supported by data | Observed cost | Recommendation |
| ---: | --- | --- | --- | --- | --- |
...
| 2 | Smart Drag Reflow | Direct drag editing on all screens | Removes induced overlap in 45/45 cases | Median of 1 other card moved, maximum of 7 | Enable by default during editing |
...
| 4 | Fisheye Focus | Local inspection of 241–364 nodes on a small screen | Restores targets to at least about 220 px | Increased local overlap and visual hierarchy occlusion | Enable on demand |
| 5 | Readable Edge Overlay | A specific connection crossing a card | Keeps the route unchanged and lets part of the hidden segment appear through the background | Local effect and subtle difference | Retain as a situational feature |

Table 5.5 Structured developer evaluation based on consistent questions

Table 5.5 does not combine different units into one total score. Instead, it gives a recommendation based on practical use. Adaptive Zoom Detail has the widest coverage and is suitable as the overview foundation. Smart Drag Reflow is the most direct feature when editing node positions. Related Node Focus is useful for checking the structure around the selected node. Fisheye Focus changes the local view most strongly on small screens and large trees, but its side effects are also clearer. Readable Edge Overlay is useful for local line occlusion, so it is closer to a supporting function.
