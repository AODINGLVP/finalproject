...
MAIN_TEX = VERSION_DIR / "latex_output" / ".build" / "zh" / "main.tex"
...
def main() -> None:
    ...
    tex = insert_after_table(tex, "五种行为树规模下的主要显示变化", figure_block("显示优化功能随树规模变化的效果", "fig:display-size-metrics", TREE))
    tex = insert_after_table(tex, "三种物理尺寸配置下的主要显示变化", figure_block("显示优化功能随屏幕尺寸配置变化的效果", "fig:display-screen-metrics", SCREEN))
