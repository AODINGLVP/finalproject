...
论文只比较第三章定义的六种显示条件。Compact、Semantic Zoom、Search、Subtree Focus 和 Collapse 都可以单独开启和关闭；Baseline 关闭这些实验功能。切换条件时，插件会清除上一个条件留下的卡片尺寸、搜索、聚焦和折叠状态，再重建画布。

\subsection{信息密度控制}
Compact 缩小卡片，但不隐藏节点。Semantic Zoom 根据当前缩放比例决定显示多少字段，不改变树的结构。Optimized Overview 同时使用这两项功能，并在 50\% 缩放下显示最低限度的信息。
...
Optimized Search 在 Overview 状态下搜索唯一的节点标题，淡化其他卡片，并把目标移入当前视口。Subtree Focus 只显示指定分支和必要上下文。Context Collapse 保留折叠根和摘要，但暂时不显示其后代。Focus 和 Collapse 都不会删除资源节点或改变父子关系。
