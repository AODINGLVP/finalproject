...
\begin{description}
    ...
    \item[显示数据：] \path{testgame/testgame/test_results/display_optimization_raw.csv}
...
协议在可玩 241 节点树上包含六种显示条件和每种三个任务。参与者先完成三项标准化练习，再进行 18 个平衡正式试验。六组 Action/路径目标和六个 Decorator 目标与条件顺序共同平衡，使计划的 12 名参与者中每个方法--目标组合恰好出现两次。工作簿包含 216 行，记录上限完成时间、误选、Zoom、Pan、成功率、可读性和认知负荷；分析脚本报告中位数与四分位数，在完整参与者少于八人时停止推断，并仅在录入真实数据后执行预注册的重复测量检验。当前没有参与者数据。使用前必须获得所需伦理许可，与导师确认流程，并保证同意和退出程序符合学校要求。

\chapter{初稿完成清单}

正式提交前，作者应：替换身份与导师占位符；按学校政策修改声明并如实说明 AI 协助；在 Tabula 核实截止时间和词数规则；完成或正式移除人体实验并同步更新摘要、结果、讨论和结论；请导师审阅文献、研究问题和证据边界；按院系要求执行 Turnitin；用模板编译并逐页检查；加入最终词数和要求的仓库/归档信息。
