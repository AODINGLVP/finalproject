...
The aim of this dissertation is to design, implement and evaluate a Godot 4.6 visual behaviour-tree plugin that supports real non-player character (NPC) execution while providing robust, switchable mechanisms for inspecting large trees.
...
\begin{description}
    ...
    \item[RQ2:] How do independently switchable visualisation techniques affect measurable properties of 31-, 121- and 364-node behaviour trees, including visible-node ratio, card area, information fields, target dimming, overview coverage and update cost?
...
RQ2 deliberately asks about measurable display properties rather than claiming improved usability. Whether the techniques reduce user completion time, error rate or cognitive load requires participant data. The corresponding study is designed in this dissertation, but no participant outcomes are fabricated.
...
\begin{enumerate}
    ...
    \item evaluate correctness, visual geometry, visual regressions, runtime performance and clean installation with reproducible evidence.
\end{enumerate}
...
\begin{itemize}
    ...
    \item a reproducible evaluation corpus containing deterministic 31-, 121- and 364-node resources, GPU screenshots, raw CSV measurements, formula workbooks and a participant-study protocol;
