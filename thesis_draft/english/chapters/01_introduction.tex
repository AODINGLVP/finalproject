...
The aim of this dissertation is to design, implement and evaluate a Godot 4.6 visual behaviour-tree plugin that supports real non-player character (NPC) execution while providing measurable controls for the visual and interaction complexity of large trees.
...
\begin{description}
    ...
    \item[RQ2:] Across controlled tree sizes and physical display sizes, how do independently switchable visualisation techniques change measurable presentation properties and fixed-path authoring interaction while preserving saved structure and execution order?
...
RQ2 deliberately concerns instrumented presentation and interaction evidence. Geometry, salience, overlap, physical-surface coverage and fixed-input processing time can establish what the editor changes as tree scale or available display area increases. They cannot establish whether people understand the tree faster, make fewer errors or experience lower cognitive load. A balanced participant protocol is retained as optional future construct validation, but it is not treated as completed evidence and no participant outcome is fabricated.
...
\begin{enumerate}
    ...
    \item evaluate correctness, visual geometry, physical-display-size robustness, visual regressions, cross-scale drag interaction and clean installation with reproducible raw data, while retaining a balanced developer-task protocol for optional future human validation.
\end{enumerate}
...
\begin{itemize}
    ...
    \item a reproducible evaluation corpus containing deterministic 31-, 61-, 121-, 241- and 364-node resources, a playable 241-node tree, real-\ac{GPU} screenshots, 900 repeated display observations, 200 versioned drag observations and 270 physical-screen observations across three connected displays, together with raw CSV files and deterministic analysis scripts;
